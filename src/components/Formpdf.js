    ...
    useEffect(() => {
        WebViewer(
            ...
            instance.setHeaderItems(header => {
              header.push({
                  type: 'actionButton',
                  img: 'https://img.icons8.com/nolan/2x/save.png',
                  onClick: async () => {
                     let data = await annotManager.exportAnnotations({ links: false, widgets: false })
                     let response = await fetch('http://localhost:6001/api/add',{
                       method:'POST',
                       body:JSON.stringify(data),
                     })
                     console.log("🚀 ~ file: Formpdf.js ~ line 30 ~ onClick: ~ response", response)
